...
def midtimediff(start_time, end_time):
    ...
    return "{:02d}:{:02d}".format(int(timestamp_minutes), int(timestamp_seconds))
...
def timediffsec(start_time, end_time):
    ...
    start_total_seconds = start_minutes * 60 + start_seconds
    end_total_seconds = end_minutes * 60 + end_seconds
    return abs(start_total_seconds - end_total_seconds)
...
def converttopdf(dirname, output_pdf='output.pdf'):
    imgs = []
...
def textonImage(image, text, font, font_size, thickness):
    ...
    wrapped_text = textwrap.wrap(text, width=int(image.shape[1] / char_width))
